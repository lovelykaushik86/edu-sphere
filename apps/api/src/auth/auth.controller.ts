...
@ApiTags('Authentication') @ApiBearerAuth() @Controller({ path: 'auth', version: '1' }) export class AuthController {
  ...
  @Get('me') @UseGuards(JwtAuthGuard) me(@Req() req: any) { return this.auth.me(req.user.sub); }
  @Get('login-history') @UseGuards(JwtAuthGuard) history(@Req() req: any) { return this.auth.loginHistory(req.user.sub); }
